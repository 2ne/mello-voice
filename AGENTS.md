...
When the user wants to **ship** a version, read **`.cursor/skills/mello-voice-release/SKILL.md`** and run the full flow: **`npm run verify`**, **`npm run release:prepare`**, write **`releases/vX.Y.Z.md`** (tone: **`releases/v1.0.0.md`**), update **`CHANGELOG.md`**, commit, **`git tag vX.Y.Z`**, push commit + tag, **`npm run setup:whisper`**, **`npm run tauri build`**, then **`npm run release:publish -- vX.Y.Z`**, then commit/push **`landing/`** download links. **Windows installers only** — no macOS release builds. Human-oriented summary: **`RELEASE.md`**.
